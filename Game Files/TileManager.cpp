...
void CTileManager::LoadEmptyMap(char** customMap) {
    m_nHeight = 9;
    m_nWidth = 16;
...
void CTileManager::LoadMap(char** customMap) {
    m_nHeight = 9;
    m_nWidth = 16;
...
void CTileManager::LoadMap(char* filename){
  ...
  for(size_t i=0; i<m_nHeight; i++){
    for(size_t j=0; j<m_nWidth; j++){
      ...
      m_chMap[i][j] = c; //load character into map
  ...
  for(int i=top; i<=bottom; i++) //for each column
    for(int j=left; j<=right; j++){ //for each row
      ...
      switch(m_chMap[i][j]){ //select which frame of the tile sprite is to be drawn
        case 'P': desc.m_nCurrentFrame = 0; break; //path
        case 'T': desc.m_nCurrentFrame = 1; break; //placeable tower tile
        case 'Y': desc.m_nCurrentFrame = 2; break; //non-placeable tower tile
        case 'Z': desc.m_nCurrentFrame = 3; break; //non-placeable tower tile
        default:  desc.m_nCurrentFrame = 4; break; //blank tile
      } //switch
...
const bool CTileManager::Visible(const Vector2& p0, const Vector2& p1, float r) const{
  if ((p1.x - p0.x) * (p1.x - p0.x) + (p1.y - p0.y) * (p1.y - p0.y) <= r * r) {
      return true;
  }
  else {
      return false;
...
char CTileManager::GetTile(int x, int y) {
	return m_chMap[y][x];
}
