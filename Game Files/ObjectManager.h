  ...
  public CCommon
{
  ...
  public:
    ...
    CObject* createEnemy(const Vector2&, int type); ///< Create new object.
    int getNumBullets();
    int GetNumEnemies();
	
	Vector2 GetEnemyPosition(CTower* tower);
	float GetEnemyRadius();

    virtual void draw(int tileColor); ///< Draw all objects.

    void FireGun(CObject*, int type, eSprite); ///< Fire object's gun.
